  ...
  return (
    <>
      <Head>
        ...
        <meta name="twitter:image" content="https://i.ibb.co/mCc3h46w/logo.png" />
          ...
          <nav className="flex space-x-4 text-sm sm:text-md mt-4">
            {(['sport', 'homme', 'femme', 'outils']).map(cat => (
              <button
                ...
                className={`transition px-2 py-1 rounded ${category === cat ? 'bg-yellow-400 text-black font-bold' : 'hover:text-yellow-400'}`}
              >
                {t(cat)}
              </button>
            ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6 md:gap-8">
            {displayedProducts.map(product => (
              <div key={product.id} className="bg-white rounded-xl shadow-lg p-3 text-black transform transition duration-300 hover:scale-105 hover:shadow-2xl">
                <img src={product.image} alt={product.title ?? product.name} className="w-full h-40 sm:h-44 md:h-48 object-contain rounded" />
                <h3 className="mt-2 font-bold text-sm sm:text-base">{product.title ?? product.name}</h3>
